...
class TollyWoodViewController: UIViewController {
        ...
        session = URLSession.shared.dataTask(with: requestURL, completionHandler: { [self] (data, response, err) in
                        ...
                        if industry == "Tollywood"
                        {
                            ...
                            var dataObj = URLSession.shared.dataTask(with: URL(string: "https://services.brninfotech.com/tws/\(movieDataConvert[poster].posters![0])".replacingOccurrences(of: " ", with: "%20"))!) { (data, res, err) in
                                DispatchQueue.main.async { [self] in
                                    ...
                                    self.imageViews.frame = CGRect(x: 20, y: 30+105*poster, width: 150, height: 100)
                                    ...
                                    label = UILabel()
                                    label.frame = CGRect(x: 230, y: 30+105*poster, width: 150, height: 100)
                                    label.text = title
            ...
            else
            {
                ...
                DispatchQueue.main.async {
                    ...
                    self.label = UILabel()
                    self.label.text = "! \(err?.localizedDescription) please restart the app"
        ...
        for poster in 0..<movieDataConvert.count
        {
            ...
            if industry == "Tollywood"
            {
                ...
                var dataObj = URLSession.shared.dataTask(with: URL(string: "https://services.brninfotech.com/tws/\(movieDataConvert[poster].posters![0])".replacingOccurrences(of: " ", with: "%20"))!) { (data, res, err) in
                    ...
                    {
                        self.label = UILabel(frame: CGRect(x: 20, y: 250, width: self.view.frame.size.width, height: 200))
                        self.label.text = "Sorry! There are no Movies"
    ...
    var tag:Int!
     ...
     @objc func tapAction(tap:UITapGestureRecognizer)
     {
        ...
        let tollyData = (storyboard?.instantiateViewController(identifier: "BollyWood1"))! as BollyWood
        tollyData.modalPresentationStyle = .fullScreen
        tollyData.movieDetails = movieDataConvert[tag]
        //var movieData = movieDataConvert[tag]
        self.navigationController?.pushViewController(tollyData, animated: true)
